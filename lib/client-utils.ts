 ...
 */
function isAtRisk(project: Project, all: Milestone[]): boolean {
  const oneWeek = 7 * 24 * 60 * 60 * 1000
  const now = Date.now()

  return all.some((m) => {
    if (m.project_id !== project.id || !m.due_date) return false
    const due = new Date(m.due_date).getTime()
    const status = norm(m.status)
  ...
  switch (sortOrder) {
    ...
    case "overdue": {
      const only = sortedCopy.filter((p) =>
        milestones.some(
          (m) => m.project_id === p.id && isOverdueMilestone(m),
        ),
      )
      return only.sort(
        (a, b) =>
    ...
    case "at-risk": {
      // ✅ Now respects backend `status = 'at-risk'` PLUS milestone conditions
      const only = sortedCopy.filter((p) => isAtRisk(p, milestones))

      // Optional ordering: earlier end_date first, then newest created
    ...
    case "completed": {
      const only = sortedCopy.filter((p) => {
        ...
        if (statusFromMilestones !== "pending") {
          // has milestones → completed only when all completed
          return statusFromMilestones === "completed"
        }
        // no milestones → fall back to project.status
        return normStatus(p.status) === "completed"
      })
      return only.sort(
        (a, b) =>
